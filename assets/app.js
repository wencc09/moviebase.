...
async function loadTopLikedPosts(limit = 3){
  const box = document.getElementById("topLikedBox");
  if(!box) return;

  // 用你現有 CONFIG / GAS_WEBAPP_URL
  const url = (window.CONFIG && CONFIG.GAS_WEBAPP_URL) ? CONFIG.GAS_WEBAPP_URL : "";
  if(!url){
    box.innerHTML = `<div class="muted">找不到 GAS_WEBAPP_URL</div>`;
    return;
  }
  ...
  try{
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ action:"posts.topLiked", limit, _t: Date.now() })
    });
    ...
    if(!items.length){
      box.innerHTML = `<div class="muted">目前還沒有熱門貼文</div>`;
      return;
    }
    ...
    items.forEach((p, idx)=>{
      ...
      div.className = "recCard"; // 直接沿用你 records 的卡片玻璃樣式
      const img = (p.photoUrls && p.photoUrls[0]) ? `<img src="${p.photoUrls[0]}" style="width:100%;height:120px;object-fit:cover;border-radius:12px;margin-top:8px;">` : "";
      const text = String(p.content || "").trim();
      const preview = text.length > 80 ? text.slice(0,80) + "…" : text;

      div.innerHTML = `
        <div class="recMeta"><span>TOP ${idx+1}</span><span>♥ ${p.likeCount || 0}</span></div>
        <div class="recTitle">${escapeHtml(p.authorName || "")}</div>
        <div class="recNote">${escapeHtml(preview)}</div>
        ${img}
      `;

      // 你如果有貼文牆可以跳轉，就在這裡改成你的路由：
      // div.addEventListener("click", ()=> location.href = `app.html#wall&post=${encodeURIComponent(p.postId)}`);
      box.appendChild(div);
    });
...
window.addEventListener("load", ()=>{
  loadTopLikedPosts(3);
  const btn = document.getElementById("btnTopLikedReload");
  if(btn) btn.addEventListener("click", ()=> loadTopLikedPosts(3));
});
